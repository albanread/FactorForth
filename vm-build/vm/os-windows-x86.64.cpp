...
// Build the SEH trampoline + unwind-info + RUNTIME_FUNCTION and
// register them with the OS so faults inside JIT'd code can find
// Factor's exception_handler.  Separated from c_to_factor_toplevel
// so the same registration covers the embedded entry points
// (factor_eval_string, factor_yield, factor_sleep) — they call
// alien-callbacks directly as function pointers and would
// otherwise have no SEH coverage, so a stack-guard page-fault
// (e.g. data-stack underflow) would crash the process instead of
// being recovered by Factor's normal error machinery.
void factor_vm::install_seh_table() {
...
void factor_vm::uninstall_seh_table() {
  seh_data* seh_area = (seh_data*)code->seh_area;
  RUNTIME_FUNCTION* func = &seh_area->func;
