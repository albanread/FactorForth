...
HMODULE hFactorDll;
HMODULE hHostExe;   // the embedding host's .exe (set in init_ffi)
...
void factor_vm::init_ffi() {
  // Get the HMODULE of *factor.dll itself* — i.e. the module that contains
  // init_ffi.  The original code did `GetModuleHandle(NULL)` which returns
  // the host executable's HMODULE; that's only correct when this VM is built
  // as factor.exe / factor.com (i.e. the executable IS the VM).  In the
  // embedded-DLL scenario it returns the embedding host's HMODULE, and every
  // subsequent ffi_dlsym(NULL, "primitive_xyz") fails because the host exe
  // doesn't export Factor primitives.  This is the root cause of issue
  // #2658's broken embedding.
  //
  // Using GetModuleHandleExW with GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS
  // gives us the module containing the supplied address — and an address
  // inside this very function definitively lives in factor.dll's code
  // section, so the result is always factor.dll's HMODULE regardless of
  // whether the VM is embedded or standalone.
  //
  // GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT means we don't increment
  // the reference count (which we'd need to release later); we just borrow
  // the handle for the life of the DLL.  Safe because factor.dll stays
  // loaded until process exit.
  // Any address in this translation unit works for FROM_ADDRESS — we just
  // need a pointer whose containing module is factor.dll.  A static here
  // serves; can't use &factor_vm::init_ffi because pointer-to-member-function
  // is not convertible to LPCWSTR.
  static int anchor;
  BOOL ok = GetModuleHandleExW(
      ...
      &hFactorDll);
  if (!ok || !hFactorDll)
    fatal_error("GetModuleHandleExW(FROM_ADDRESS) failed", GetLastError());

  // ALSO grab the embedding host's exe handle.  When `ffi_dlsym`
  // gets a library==NULL request and factor.dll doesn't have the
  // symbol, this is the second place we look — that's where the
  // host's `rt_*` callbacks and other embed-time-exported symbols
  // live.  GetModuleHandle(NULL) at this point returns the host
  // .exe handle even when called from inside factor.dll (per
  // MSDN: "a handle to the file used to create the calling
  // process").  Safe to fail-soft on this one; if the host has
  // no exports we never try this path.
  hHostExe = GetModuleHandle(NULL);
}
...
cell factor_vm::ffi_dlsym(dll* dll, symbol_char* symbol) {
  // If the caller specified an explicit library (e.g. opened via
  // `add-library`/dlopen), look there only.  Embedding clients
  // that want host-exposed symbols pass a library with dll==NULL
  // (i.e., the dlsym primitive's library arg is `f`); they get
  // both factor.dll AND the host .exe searched, in that order.
  if (dll) {
    return (cell)GetProcAddress((HMODULE)dll->handle, symbol);
  }
  cell result = (cell)GetProcAddress(hFactorDll, symbol);
  if (result == 0 && hHostExe && hHostExe != hFactorDll) {
    result = (cell)GetProcAddress(hHostExe, symbol);
  }
  return result;
}
