\ gfx-mandelbrot.f - the Mandelbrot set rendered to a pane
\
\ Pure-Forth port of WF64's MASM `fractal-iter` primitive.
\ Everything here runs through FactorForth's compiler;
\ Factor's polymorphic + - * / dispatch on the typed stack
\ values so the same `f+ f- f*` we write resolve to IEEE
\ doubles without a separate FP-stack ceremony.
\
\ Try it:  gfx-mandelbrot
\
...
\ ── 2. Escape-time iteration (the heart of the renderer) ──
\
...
: mb-bounded? ( -- ? )
    \ |z|² = x² + y² < 4 means the orbit is still bounded.
    \ (Treating |z|²=4 as escape is benign — the boundary is
    \ a measure-zero set of starting points.)
    mb-x f@ mb-x f@ f*
    mb-y f@ mb-y f@ f*
    f+
    4e f< ;
...
: fractal-iter ( z0x z0y cx cy maxiter -- n )
    ...
    \ pred is true.  Pred is true iff we're still inside the
    \ |z|<2 disk AND we haven't burnt the iteration budget.
    \ When either fails, fall through to the count read.
    begin
        mb-bounded?  mb-have-budget?  and
    while
        mb-step
    repeat
...
: mb-colour ( n -- rgb )
    \ Fast-path: pixels inside the set (orbit never escaped) get
    \ solid black.  `exit` here lowers cleanly because the compiler's
    \ `lower_exit` pass tail-inlines the post-IF case-expression
    \ into the ELSE branch at AST level, so no `with-return`
    \ wrap is generated and Factor's JIT optimises the whole word.
    dup mb-maxiter = if drop 0x000000 exit then

    \ Escape-time gradient.  `15 and` reduces the count to a 4-bit
    \ palette index; every match arm pushes one colour, and the
    \ trailing literal acts as the default for the (statically
    \ unreachable) no-match path that Factor's effect checker
    \ otherwise considers an unbalanced branch.
    15 and case
        0  of 0x0D1540 endof
        1  of 0x102B80 endof
        2  of 0x1558C8 endof
...
." Loaded: gfx-mandelbrot.  Try 'gfx-mandelbrot'" cr
